...
const LinkedInStatusCard = ({ account, loading, onConnect, onDisconnect }) => {
  if (loading) {
    return (
  ...
  return (
    ...
    >
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
        <Box sx={{ color: '#0A66C2', display: 'flex' }}>
          ...
          {isConnected ? (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 0.5 }}>
              ...
              <Typography variant="caption" sx={{ color: '#64748b', fontSize: '0.75rem' }}>
                Connected as {account?.profileData?.name || account?.profileData?.username || 'LinkedIn User'}
              </Typography>
              <Chip
      ...
      <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
        {isConnected ? (
          ...
          <Button
            ...
            onClick={onConnect}
            sx={{ borderRadius: 2, fontSize: '0.75rem', bgcolor: '#0A66C2', '&:hover': { bgcolor: '#004182' } }}
          >
            Connect LinkedIn
          </Button>
        )}
